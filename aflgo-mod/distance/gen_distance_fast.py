...
MODIFIED: Complexity-based edge weighting.
- Reads block complexity from BBnames.txt (format: bb_name,complexity).
- For CFG dot files, increases weight of edges leading to complex blocks.
...
def load_block_complexities(bbnames_path):
    ...
    complexities = {}
    if not bbnames_path.exists():
        return complexities
    with bbnames_path.open("r") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            ...
            if len(parts) >= 2:
                ...
                try:
                    ...
                    complexity = 0
                if complexity > 0:
                    complexities[bb_name] = complexity
    return complexities
